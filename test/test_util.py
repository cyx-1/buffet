from util import download_file_and_compare, process_data_from_fred, Status, create_dual_axis_plot
import os
import pandas as pd
import numpy as np
import matplotlib
...
def test_process_data_from_fred_success(mocker):
    ...
    mocker.patch(
        'util.download_file_and_compare',
        return_value=Status(success=True, result='New file downloaded')
    )
    ...
    mocker.patch('os.path.exists', side_effect=lambda path: 
                 path == './temp/test_unemployment.csv' or original_exists(path))
    ...
    status = process_data_from_fred(
        'https://test.url',
        'test_unemployment.csv',
...
def test_process_data_from_fred_error_cases(mocker):
    ...
    status = process_data_from_fred(
        'https://test.url',
        'test_unemployment.csv',
        ['observation_date', 'INVALID_COLUMN'],
        './temp'
    )
    ...
    mocker.patch(
        'util.download_file_and_compare',
        return_value=Status(success=False, result='Failed with status code 404')
    )
...
def test_create_dual_axis_plot():
    ...
    df1 = pd.DataFrame({
        ...
        'value1': rng.uniform(3, 6, len(dates))  # Simulated unemployment data
    })
    df2 = pd.DataFrame({
        'observation_date': dates,
        'value2': rng.uniform(20000, 25000, len(dates))  # Simulated GDP data
    })
